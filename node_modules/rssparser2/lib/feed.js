...
function formatRSS(json) {
    ...
    if (!channel.pubDate && !channel.pubDate && channel.item){
        ...
        _.each(channel.item, function (val, index) {
            if (val.pubDate){
                pubDateTimestamps.push(Date.parse(val.pubDate[0]))
            }
        });

        channelPubDate = Math.max.apply(Math,pubDateTimestamps);
        output.last_modified = channelPubDate;
        output.update = channelPubDate;
    }
...
function formatATOM(json) {
    ...
    if (channel.entry) {
        ...
        _.each(channel.entry, function (val, index) {
            ...
            if (val.content) {
                obj.content = val.content[0]._;
            }else {
                obj.content = "";
            }

            if (val.summary) {
                obj.summary = val.summary[0]._;
            }else {
                obj.summary = "";
            }
